...
%             self.angular_ctrl = PIDControl(P.kp,P.ki,P.kd,P.limit,P.beta,dt);
        ...
        function self = move_to_target(self)
            % Moves the enemy agent toward the goal position
            
            for i = 1:length(self)
                % Determine the required velocities
                self(i).calculateVelocity();
        ...
        function self = setTarget(self,target)
            ...
            % the goal position based on some enemy goal. This assumes the
            % target is passed as a column vector.
        ...
        function self = animate_one(self)
